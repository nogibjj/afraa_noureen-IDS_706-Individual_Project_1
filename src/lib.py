...
def display_contries_info(countries_info_df):
    ...
    environmental factors, healthcare metrics and education statistics.
    """
...
def infant_mortality(countries_info_df):
    ...
    """
    high_infant_mortality_df = countries_info_df.query(
        "`Infant mortality` == `Infant mortality`.max()"
        )
